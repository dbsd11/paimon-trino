...
import static org.apache.paimon.shade.guava30.com.google.common.base.MoreObjects.toStringHelper;
...
public abstract class AbstractOrcDataSource implements OrcDataSource {
    ...
    protected abstract void readInternal(
            long position, byte[] buffer, int bufferOffset, int bufferLength) throws IOException;
    ...
    public Slice readTail(int length) throws IOException {
        ...
        readTimeNanos += Math.abs(System.nanoTime() - start);
    ...
    private <K> Map<K, OrcDataReader> readSmallDiskRanges(Map<K, DiskRange> diskRanges)
        ...
        if (options.isLazyReadSmallRanges()) {
            for (DiskRange mergedRange : mergedRanges) {
                ...
                for (Entry<K, DiskRange> diskRangeEntry : diskRanges.entrySet()) {
                    ...
                    if (mergedRange.contains(diskRange)) {
                        slices.put(
                                diskRangeEntry.getKey(),
                                new MergedOrcDataReader(id, diskRange, mergedRangeLazyLoader));
                    }
                }
            }
        } else {
            ...
            for (Entry<K, DiskRange> entry : diskRanges.entrySet()) {
                slices.put(
                        entry.getKey(),
                        new MemoryOrcDataReader(
                                id,
                                getDiskRangeSlice(entry.getValue(), buffers),
    ...
    private final class MergedOrcDataReader implements OrcDataReader {
        private final OrcDataSourceId orcDataSourceId;
        private final DiskRange diskRange;
        private final LazyBufferLoader lazyBufferLoader;
        private Slice data;

        public MergedOrcDataReader(
            ...
            if (data == null) {
                ...
                if (data.length() != diskRange.getLength()) {
                    throw new OrcCorruptionException(
                            id,
                            "Expected to load %s bytes, but %s bytes were loaded",
                            diskRange.getLength(),
                            data.length());
                }
            }
    ...
    private class DiskOrcDataReader extends AbstractDiskOrcDataReader {
        private final DiskRange diskRange;
